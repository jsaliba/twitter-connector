...
public class GetRetweetedByIdsTestCases extends TwitterTestParent {
	...
	public void testGetRetweetsByIdsDefaultValues() {
		...
		try {
			...
			flow = lookupFlowConstruct("get-retweeted-by-ids-default-values");
			response = flow.process(getTestEvent(aRetweetedStatus.getId()));

			IDs responseList = (IDs) response.getMessage().getPayload();
			...
			assertTrue(userNotFoundErrorMessage, TwitterTestUtils.isUserIdOnIdList(responseList,expectedUserId));
