...
public class GetRetweetedByUserByUserIdTestCases extends TwitterTestParent {
	...
	@Test
	public void testGetRetweetedByUserByUserIdDefaultValues() {
		
		TestStatus aRetweetedStatus = (TestStatus) testObjects.get("firstRetweetedStatus");
		
		try {
			
			flow = lookupFlowConstruct("get-retweeted-by-user-by-user-id-default-values");
			response = flow.process(getTestEvent(sandbox.getUserId()));
	...
	@Test
	public void testGetRetweetedByUserByUserIdParametrized() {
		...
		operationParams.put("userId", sandbox.getUserId());
		
		try {
			
			flow = lookupFlowConstruct("get-retweeted-by-user-by-user-id-parametrized");
